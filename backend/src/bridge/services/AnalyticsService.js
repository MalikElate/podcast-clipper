...
export class AnalyticsService {
  ...
  aggregate(rows) {
    ...
    values.engagement = availableInteractions.length ? availableInteractions.reduce((sum, value) => sum + value, 0) : null;
  ...
  async syncDelivery(delivery) {
    ...
    try {
      ...
      this.store.put("delivery", { ...current, metrics: this.normalize(result.values), metricsUpdatedAt: this.clock(), metricsAttemptedAt: this.clock(), metricsError: null, metricsNote: result.unavailableReason || result.note || null });
    } catch (error) {
      const current = this.store.get("delivery", delivery.id);
  ...
  async refresh(uid, projectId, { postId, accountId } = {}) {
    ...
    for (const delivery of deliveries.sort((a, b) => (a.metricsAttemptedAt || 0) - (b.metricsAttemptedAt || 0)).slice(0, 50)) await this.syncDelivery(delivery);
    return this.report(uid, projectId);
  }
  ...
  async tick() {
    ...
    this.running = true;
    try {
      const deliveries = this.store.list("delivery", { status: "published" }).filter(item => !item.metricsAttemptedAt || item.metricsAttemptedAt < this.clock() - 15 * 60000).sort((a, b) => (a.metricsAttemptedAt || 0) - (b.metricsAttemptedAt || 0));
      for (const delivery of deliveries.slice(0, 10)) await this.syncDelivery(delivery);
    } finally { this.running = false; }
  }

  report(uid, projectId) {
    this.projects.require(uid, projectId);
    const posts = this.posts.list(uid, projectId).map(post => ({ ...post, totals: this.aggregate(post.deliveries.filter(delivery => delivery.status === "published").map(delivery => delivery.metrics || {})) }));
    ...
    const accounts = this.accounts.list(uid, projectId).map(account => {
      const accountPosts = posts.filter(post => post.deliveries.some(delivery => delivery.accountId === account.id)).map(post => ({ id: post.id, title: post.title || post.caption || post.media[0]?.filename || "Untitled post", createdAt: post.createdAt, delivery: post.deliveries.find(delivery => delivery.accountId === account.id), totals: this.aggregate(post.deliveries.filter(delivery => delivery.accountId === account.id && delivery.status === "published").map(delivery => delivery.metrics || {})) }));
      return { ...account, posts: accountPosts, totals: this.aggregate(deliveries.filter(delivery => delivery.accountId === account.id).map(delivery => delivery.metrics || {})) };
    });
    return { posts, accounts, totals: this.aggregate(deliveries.map(delivery => delivery.metrics || {})), publishedCount: deliveries.length, postCount: posts.length,
      engagementDefinition: "Likes + comments + shares + saves, where reported. Views are separate. Totals sum activity across accounts and may include the same person more than once." };
  }
}
