...
export class PrivacyService {
  ...
  prune() {
    ...
    for (const delivery of this.store.list("delivery", { limit: null })) {
      if (delivery.platform === "pinterest" && (delivery.metrics || delivery.metricsHistory?.length || delivery.metricsUpdatedAt || delivery.metricsNote || delivery.metricsError)) this.store.put("delivery", { ...delivery, metrics: null, metricsHistory: [], metricsUpdatedAt: null, metricsAttemptedAt: null, metricsNote: null, metricsError: null });
      if (delivery.platform === "youtube" && (delivery.metricsUpdatedAt || delivery.publishedAt || delivery.createdAt) < this.clock() - 30 * DAY && (delivery.metrics || delivery.metricsHistory?.length || delivery.externalId || Object.keys(delivery.progress || {}).length)) this.store.put("delivery", { ...delivery, metrics: null, metricsHistory: [], metricsUpdatedAt: null, externalId: null, url: null, progress: {}, contentSnapshot: null, metricsNote: "YouTube data expired. Open YouTube Studio to view this video's current status." });
    }
