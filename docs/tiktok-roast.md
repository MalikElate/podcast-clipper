...
The scrolling walkthrough shows each post beside a caption-specific roast, evidence, and improvement. On desktop the current commentary follows the visible post; on mobile it appears inside each post card. Previous/next controls and an optional 9-second walkthrough advance through the sample. Manual scrolling, video playback, page hiding, or Pause stops the walkthrough. Reduced-motion users get immediate scroll transitions. Only the requested post mounts TikTok’s [official embedded player](https://developers.tiktok.com/docs/en/embed-player); changing active posts unloads it. External post links remain available when previews or playback fail.
