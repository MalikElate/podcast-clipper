...
- `POST /api/free-tools/generate` accepts bounded topic text and returns caption or tag suggestions. It uses the existing `ROAST_AI` binding and the same `ROAST_BUDGET` `daily` object as the roast: a combined maximum of 100 AI calls per UTC day. Unavailable or invalid AI responses produce explicitly labeled template/keyword suggestions. The browser can also generate those fallbacks if the request fails.
