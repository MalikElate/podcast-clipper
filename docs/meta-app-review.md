...
- `wrangler secret list` showed none of the six Meta publishing credential variables below. Live connection and publication have not been verified.
...
- Facebook callback saved; Meta's validator confirmed it is valid. Instagram Login callback saved and visible in its business login settings.
...
- Threads redirect callback was entered, but saving returned **Form can't be saved — Please verify all information is entered correctly and try again**, with no field-specific error. Reload confirmed that the callback was not saved. Meta's Threads setup documentation also lists uninstall and deletion callback URLs as setup fields.
- Implemented product-specific signed removal/deletion handlers, encrypted authorization identity mapping, stale-OAuth protection, durable acknowledgment, and private deletion-status receipts. The backend suite passed 173 tests; this is local evidence, not a live Meta callback test. Runtime credentials and dashboard callback registration are still pending.
...
- Production credential entry is in progress. The Facebook app secret was retrieved without displaying it; Meta requested further password confirmation for the Threads secret, and the user was asked to complete that dialog. No production secret deployment or review submission is confirmed yet.
...
1. Complete the open Meta password confirmation, retrieve the remaining product secrets securely, and finish the staged Cloudflare runtime configuration. The draft currently contains only the Facebook credential pair; it has not been deployed.
...
3. Deploy the tested callback handlers and register their actual URLs for each product. Resolve the Threads settings validation error and verify saved OAuth redirect persistence.
