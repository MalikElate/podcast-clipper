...
- Google and TikTok revocation uses encrypted credentials in a separate retry job. Local profile and publishing data normally do not wait for remote revocation. If active work or a stuck lease prevents ordinary cleanup, maintenance force-purges the local connection data and any retained revocation credentials after six days. This reserves a full daily wake interval before the public seven-day deadline. When revocation cannot be confirmed, credentials are destroyed and an operator receipt remains for thirty days. The user must then remove Meadow in the provider's connected-app settings. Google revocation can affect the full grant, including other Google channels or clients sharing it.
...
- Meta callback confirmation receipts contain random references and deletion job IDs, without raw platform identities or tokens. They expire after ninety days; an unfinished deletion extends its receipt. A hashed Meta subject barrier lasts twenty-four hours to reject OAuth flows started before removal. Newer authorizations survive delayed or repeated callbacks. Keep callback receipts private, avoid logging signed request bodies, and verify actual platform removal events during acceptance testing.
...
- YouTube profiles are checked daily where available. Profile caches, API metadata, and analytics are cleared when stale for thirty days without deleting a healthy attached authorization. Metrics lookup removes video API identifiers and statistics when the video is absent. Temporary provider or renewal errors keep the connection for retry. A confirmed missing channel queues deletion of that YouTube connection; loss of the underlying Google authorization queues deletion of every stored connection that shares it. YouTube is excluded from combined totals and derived engagement; raw metrics remain available per video.
