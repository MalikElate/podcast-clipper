...
2. Configure Firebase Authentication and its browser web values. Add the production hostname to authorized domains. Supply a Firebase Admin credential from the same project as a private backend secret. Compose uses `FIREBASE_SERVICE_ACCOUNT_JSON`; a file credential requires a separate read-only private mount.
