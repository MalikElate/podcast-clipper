...
### Meta removal callbacks

For each of `facebook`, `instagram`, and `threads`, register `https://findmeadow.com/api/meta/PLATFORM/deauthorize` as the deauthorize/uninstall callback and `https://findmeadow.com/api/meta/PLATFORM/data-deletion` as the data-deletion callback. Use the corresponding product's client secret in the table above. These endpoints accept a form POST containing Meta's HMAC-SHA256 `signed_request`; they return 503 until that product secret is configured. They do not replace the OAuth redirect URLs.

The callback queues erasure of the authorizing user's matching Meta connections, cancels their queued deliveries, and returns a confirmation code and a private, human-readable status URL. Original uploads, Meadow account data, unrelated connections, and already-published social posts remain. Replayed or delayed callbacks do not erase a newer grant. Facebook discovery records the authorizing app-scoped user ID alongside the encrypted Page token; any legacy Facebook connections without that identity must reconnect before callback deletion can be verified. See [Meta's callback contract](https://developers.facebook.com/documentation/development/create-an-app/app-dashboard/data-deletion-callback).

Pinterest requires an approved business-account developer app. Set `PINTEREST_ENVIRONMENT=sandbox` while testing with Trial access; this routes OAuth token exchange and API requests to Pinterest's Sandbox. Sandbox entities are visible only to their creator and video Pins are unavailable there. Remove the variable when the app has Standard access and is ready for production, then reconnect all Pinterest accounts and reselect their boards. Sandbox and Production tokens are not interchangeable. Use the prepared [Pinterest app setup and review](pinterest-app-review.md) values and demo checklist.

For LinkedIn organizations, add the approved `rw_organization_admin`, `w_organization_social`, and required read scopes to `LINKEDIN_SCOPES`; the adapter then enumerates eligible organizations. Member read analytics require restricted access and are not granted merely by Share on LinkedIn. The current request header defaults to `LinkedIn-Version: 202607`; keep it on a supported version as the platform retires versions. Member image status checks use LinkedIn's documented legacy read path because versioned Images GET rejects write-only member access. See [LinkedIn setup](linkedin-app-setup.md), [Posts API](https://learn.microsoft.com/en-us/linkedin/marketing/community-management/shares/posts-api?view=li-lms-2026-07), and [Images permissions](https://learn.microsoft.com/en-us/linkedin/marketing/community-management/shares/images-api?view=li-lms-2026-07#permissions).
...
Production enables `TIKTOK_DIRECT_POST_PRIVATE_ONLY=true` until the Direct Post audit is approved. Direct posts offer only **Only me** (`SELF_ONLY`), and the backend rejects every other audience. Users must still select that audience explicitly. Existing queued posts or retries with another audience stop with an actionable error; their saved audience is not silently changed. Posts TikTok already accepted continue status reconciliation. Inbox uploads are unaffected.

For unaudited testing, TikTok also requires the creator account itself to be private; selecting Only me does not change the account's privacy setting. After TikTok approves the Direct Post audit, set `TIKTOK_DIRECT_POST_PRIVATE_ONLY` to `false` in `wrangler.jsonc` (or the backend environment when self-hosted) and redeploy. Refresh the account's publishing options before choosing an audience TikTok makes available. Paid partnerships remain incompatible with Only me.
