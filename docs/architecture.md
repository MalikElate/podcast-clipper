...
The public and authenticated interfaces share one plan catalog. `BillingService` creates Stripe subscription Checkout and Customer Portal sessions, verifies signed webhooks and owned Checkout returns, and stores customer/subscription state by Clerk user ID. Durable idempotency keys and per-user locks recover checkout retries without duplicate purchases. Subscription updates retrieve current Stripe state and map configured Price IDs rather than original Checkout metadata. Paid-plan entitlement enforcement remains a separate future service. Collaborators can be introduced through a project-access service later. Neither concern is embedded in provider code or the media pipeline.
