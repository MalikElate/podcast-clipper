...
The shell mounts `Composer`, `PostsQueue`, `Accounts`, `MediaLibrary`, `Analytics`, and `ClippingStudio` by project. Switching users or projects remounts the private workspace and aborts stale reads. The API client obtains a fresh Clerk session token for authenticated requests.
