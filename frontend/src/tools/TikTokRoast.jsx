...
const numberLabel = {strong:'Strong',thin:'Thin',filler:'Filler',unscored:'Unscored'};
  ...
  return <div className="tiktok-roast-page">
    ...
    <main>
      <section className="roast-hero" aria-labelledby="roast-title">
        <div className="roast-intro"><span className="roast-eyebrow"><span /> FREE TOOL · NO SIGNUP</span><h1 id="roast-title">Roast my <span>TikTok.</span></h1><p>Your captions are in the hot seat. Get a playful roast, an honest score, and a better idea for your next post.</p></div>
        <div className="roast-workspace">
          <div className="roast-input-card"><span className="roast-card-kicker">LET’S SEE WHAT YOU’VE BEEN POSTING</span><h2>Drop your handle.</h2><form onSubmit={roast}><label htmlFor="roast-handle">TikTok handle or profile URL</label><div className="roast-input-wrap"><span aria-hidden="true">@</span><input id="roast-handle" name="handle" value={handle} onChange={e=>setHandle(e.target.value)} placeholder="yourhandle" maxLength={180} autoCapitalize="none" autoCorrect="off" spellCheck="false" required aria-describedby="roast-input-help" disabled={busy}/></div><button className="roast-submit" disabled={busy || !handle.trim()} type="submit">{busy ? 'Preparing your roast…' : 'Roast my TikTok'}<span aria-hidden="true">{busy ? '◌' : '↗'}</span></button><p id="roast-input-help">Public profiles only. No login. Nothing gets posted.</p></form>{busy && <p className="roast-loading" role="status">Reading public captions and checking the hooks. This can take a few seconds.</p>}{error && <p className="roast-error" role="alert">{error}</p>}<div className="roast-scope"><span aria-hidden="true">✳</span><p>We roast the <strong>captions</strong>, not the person. Video, audio, and on-screen text aren’t included.</p></div></div>
      ...
      {result && <section className="roast-results" ref={output} tabIndex="-1" aria-labelledby="roast-result-title">
        <div className="roast-result-top"><div><span className="roast-eyebrow">THE VERDICT</span><h2 id="roast-result-title">@{result.profile.handle}, you’re up.</h2><p>{result.posts.length} public captions sampled · {new Date(result.sampledAt).toLocaleDateString()}</p></div><a className="roast-profile-link" href={result.profile.url} target="_blank" rel="noreferrer">View TikTok profile ↗</a></div>
        ...
        <div className="roast-share"><button onClick={()=>share('image')}>↓ Save roast card</button><button onClick={()=>share('copy')}>Copy roast + link</button><span role="status">{notice}</span></div>
        <div className="roast-fixes">{result.observations.map((o,i)=><article key={o.title}><span className="roast-fix-number">0{i+1}</span><h3>{o.title}</h3><p>{o.evidence}</p><div><strong>Try this next</strong><p>{o.fix}</p></div></article>)}</div>
        <details className="roast-receipts"><summary>Show the receipts <span>{result.counts.strong} strong · {result.counts.thin} thin · {result.counts.filler} filler{result.counts.unscored ? ` · ${result.counts.unscored} unscored` : ''}</span></summary><p className="roast-receipts-note">The same checks apply to every caption. Likes and follower counts do not affect the score.</p><div>{result.posts.map((post,i)=><article key={post.id}><div><a href={post.url} target="_blank" rel="noreferrer">Post {i+1} ↗</a><span className={`roast-bucket ${post.bucket}`}>{numberLabel[post.bucket]}</span></div><blockquote>{post.caption || 'No caption available'}</blockquote><p>{post.reason}</p></article>)}</div></details>
        <div className="roast-recovery"><div><span className="roast-eyebrow">THE COMEBACK STARTS HERE</span><h2>Better captions. Meet a better workflow.</h2><p>Turn the roast into your next post. Draft, preview, and schedule your content with Meadow.</p></div><a href={appHref('/dashboard')}>Start free with Meadow ↗</a></div>
      </section>}
      <section className="roast-explainer" aria-labelledby="roast-how-title"><div><span className="roast-eyebrow">HOW THE HEAT WORKS</span><h2 id="roast-how-title">A roast with receipts.</h2><p>Specific feedback you can use. No mystery number. No follower-count flexing.</p></div><div className="roast-steps"><article><span>01</span><h3>Drop a public handle</h3><p>We look at the captions TikTok makes available in its public profile embed.</p></article><article><span>02</span><h3>Find the weak spots</h3><p>Repeated hooks, hashtag piles, and engagement bait get called out with examples.</p></article><article><span>03</span><h3>Make the next one better</h3><p>Take the fixes, save your roast card, and put a stronger caption out into the world.</p></article></div></section>
