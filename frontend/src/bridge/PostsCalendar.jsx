...
import { isTikTokInbox } from "./deliveryPresentation.js";
...
export default function PostsCalendar({ project, onCreate }) {
  ...
  const events = useMemo(() => {
    ...
    data.posts.forEach(post => post.deliveries.forEach(delivery => {
      const at = delivery.status === "published" ? delivery.publishedAt : delivery.status === "awaiting_publish" ? delivery.deliveredAt || delivery.dueAt : delivery.dueAt;
      if (!at) return;
      const key = dateKey(at, project.timeZone);
      const item = { id: delivery.id, at, status: delivery.status, platform: delivery.platform, inbox: isTikTokInbox(delivery), title: post.title || post.caption || post.media[0]?.filename || "Media post" };
  ...
  return <>
    <div className="bridge-intro-row"><p>See publishing and TikTok transfers in one monthly view. A TikTok transfer still needs you to finish posting in the app.</p><button className="bridge-button" onClick={() => onCreate("")}><Icon name="plus" size={17}/> Create post</button></div>
    <Alert message={error}/>
    ...
    <div className="bridge-calendar bridge-panel" role="grid" aria-label={`${monthLabel(visibleMonth.year, visibleMonth.month)} post calendar`}>
      ...
      {cells.map((day, index) => {
        ...
        const content = <><span className="bridge-calendar-day-head"><span className="bridge-calendar-date">{day}</span>{canCreate && <span className="bridge-calendar-add" aria-hidden="true"><Icon name="plus" size={14}/></span>}</span><span className="bridge-calendar-events">{items.slice(0, 3).map(item => <span className={`bridge-calendar-event ${item.status}`} title={`${item.title} · ${eventTime(item.at, project.timeZone)}${item.status === "awaiting_publish" ? " · Finish in TikTok" : item.inbox ? " · Transfer to TikTok" : ""}`} key={item.id}><span>{eventTime(item.at, project.timeZone)}</span><strong>{item.status === "awaiting_publish" ? "Finish in TikTok · " : item.inbox ? "TikTok transfer · " : ""}{item.title}</strong></span>)}{items.length > 3 && <span className="bridge-calendar-more">+{items.length - 3} more</span>}</span></>;
