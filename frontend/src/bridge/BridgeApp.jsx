...
export default function BridgeApp() {
  ...
  if (!user && !localPreview) return <div className="bridge bridge-signin" data-theme="light"><div className="bridge-signin-brand"><BridgeMark/><span>bridge</span></div><Auth onBack={() => {}}/></div>;
  ...
  return <div className="bridge" data-theme="light">
    ...
    <main className="bridge-main"><header className="bridge-topbar"><button className="bridge-icon-button bridge-menu-toggle" onClick={() => setMenuOpen(true)} aria-label="Open navigation"><Icon name="menu"/></button></header>
      <div className="bridge-content"><Alert message={error}/><Alert message={notice} success/><div className="bridge-page-heading"><h1>{activeModule?.title || activeModule?.name}</h1></div>
        {!config ? <div className="bridge-panel bridge-empty"><p>{error ? "Bridge could not load. Check your connection and refresh this page." : "Loading Bridge…"}</p></div> : isConfigurationView ? <ConfigurationWorkspace user={user} project={project} config={config} view={view} onProjectUpdated={updated => setProjects(current => current.map(item => item.id === updated.id ? updated : item))}/> : !project ? SHOW_WORKSPACE_CONTROLS ? <div className="bridge-onboarding bridge-panel"><div className="bridge-empty-icon"><BridgeMark/></div><h2>Start with a workspace</h2><p>Keep a brand or client’s accounts, content, and results together.</p><button className="bridge-button" onClick={() => setProjectForm({ name: "", timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone })}><Icon name="plus" size={17}/> Create workspace</button></div> : <div className="bridge-panel bridge-empty"><div className="bridge-empty-icon"><BridgeMark/></div><h2>Bridge is getting ready</h2><p>Your publishing account is not available yet.</p></div> : <ProjectWorkspace key={project.id} project={project} config={config} view={view} navigate={navigate} compose={compose} seed={seed} scheduledDate={scheduledDate} seedVersion={seedVersion} clearSeed={() => { setSeed(null); setScheduledDate(""); }} connectionId={connectionId} clearConnection={() => setConnectionId("")} notify={setNotice}/>}
      </div>
    </main>
