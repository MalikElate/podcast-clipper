...
export default function ConfigurationSettings({ user, project, config, onProjectUpdated }) {
  ...
  return <>
    ...
    <div className="bridge-settings-grid">
      <section className="bridge-panel bridge-settings-card"><div className="bridge-settings-card-heading"><span className="bridge-settings-icon"><Icon name="profile"/></span><div><h2>Workspace profile</h2><p>The account currently signed in to Bridge.</p></div></div><div className="bridge-settings-readonly"><span className="bridge-person-avatar">{(user?.email || "D").slice(0, 1).toUpperCase()}</span><div><strong>{user?.displayName || "Dashboard"}</strong>{user?.email && <span>{user.email}</span>}</div></div></section>
      ...
      <section className="bridge-panel bridge-settings-card bridge-settings-status"><div className="bridge-settings-card-heading"><span className="bridge-settings-icon"><Icon name="check"/></span><div><h2>Service status</h2><p>Configuration reported by this Bridge server.</p></div></div><div className="bridge-status-list"><Status label="Media storage" ready={config.mediaReady}/><Status label="Social connections" ready={config.connectionsReady}/><Status label="Publishing worker" ready={config.features?.publishing}/><Status label="Clipping studio" ready={config.features?.clipping}/><Status label="Analytics" ready={config.features?.analytics}/></div></section>
    </div>
  </>;
}
