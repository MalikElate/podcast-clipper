...
export default function ConfigurationSettings({ user, project, config, onProjectUpdated, onSignOut, signingOut }) {
  const [form, setForm] = useState(project ? { name: project.name } : null);
  const [busy, setBusy] = useState(false), [error, setError] = useState(""), [notice, setNotice] = useState("");
  useEffect(() => setForm(project ? { name: project.name } : null), [project?.id, project?.name]);
  ...
  return <>
    ...
    <div className="bridge-settings-grid">
      ...
      <section className="bridge-panel bridge-settings-card"><div className="bridge-settings-card-heading"><span className="bridge-settings-icon"><Icon name="settings"/></span><div><h2>Project settings</h2><p>Used for schedules, calendars, and published timestamps.</p></div></div>{form ? <form onSubmit={save}><Field label="Project name"><input required maxLength={80} value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))}/></Field><Field label="Time zone" hint="Meadow uses your device's current time zone, so schedules follow you when you travel."><input value={project.timeZone} readOnly disabled/></Field><button className="bridge-button" disabled={busy}>{busy ? "Saving…" : "Save settings"}</button></form> : <p>Create a project to configure its publishing defaults.</p>}</section>
