...
import { canCancelRemaining, isTikTokInbox, retryConfirmation } from "./deliveryPresentation.js";
...
export default function PostsQueue({ project, accounts: initialAccounts, accountsReady = true, media, catalog, onCreate, onEditDraft, onUpload, section = "posts" }) {
  ...
  const [accountId, setAccountId] = useState(""), [search, setSearch] = useState(""), [error, setError] = useState(""), [busy, setBusy] = useState(false), [uploading, setUploading] = useState(false), [action, setAction] = useState(null), [confirmed, setConfirmed] = useState(false), [editing, setEditing] = useState(null);
  ...
  return <><div className="bridge-intro-row"><p>{current.intro}</p><button className="bridge-button" onClick={onCreate}><Icon name="plus" size={17}/> Create post</button></div><Alert message={error || loadError}/><div className="bridge-stat-grid compact"><div className="bridge-stat"><span>Waiting in queue</span><strong>{counts.queued}</strong></div><div className="bridge-stat"><span>Published deliveries</span><strong>{counts.published}</strong></div><div className="bridge-stat"><span>Posts needing attention</span><strong>{counts.attention}</strong></div></div>
    ...
    {!visible.length ? <div className="bridge-panel"><Empty icon={section === "failed" ? "failed" : section === "drafts" ? "drafts" : "all"} title={loading ? "Loading posts…" : current.empty}>{loading ? "Retrieving this project’s posts." : current.detail}</Empty></div> : <div className="bridge-post-list">{visible.map(post => <article className="bridge-panel bridge-post-card" key={post.id}><div className="bridge-post-card-heading">{post.media[0] && <div className="bridge-post-thumbnail"><MediaThumb media={post.media[0]}/></div>}<div className="bridge-grow"><h3>{post.title || post.caption.slice(0, 90) || "Media post"}</h3><p>{post.caption.slice(0, 220) || `${post.media.length} media ${post.media.length === 1 ? "item" : "items"}`}</p><small>{post.status === "draft" ? "Last saved" : "Created"} {dateTime(post.status === "draft" ? post.updatedAt || post.createdAt : post.createdAt, project.timeZone)}</small></div><Badge status={post.status}/></div><div className="bridge-deliveries">{post.deliveries.filter(item => !accountId || item.accountId === accountId).map(delivery => <div className="bridge-delivery-row" key={delivery.id}><PlatformBadge platform={delivery.platform} catalog={catalog}/><div className="bridge-grow"><strong>{delivery.accountName}</strong><span>{delivery.status === "published" ? `Published ${dateTime(delivery.publishedAt, project.timeZone)}` : delivery.status === "awaiting_publish" ? `Sent to TikTok ${dateTime(delivery.deliveredAt, project.timeZone)}` : `${isTikTokInbox(delivery) ? "Transfer to TikTok · " : ""}${dateTime(delivery.dueAt, project.timeZone)}`}{delivery.estimated ? " · estimate" : ""}</span>{delivery.status === "awaiting_publish" && <small>Open the inbox notification in TikTok to finish editing and post.</small>}{delivery.delayed && pending.includes(delivery.status) && <small>Waiting for posting allowance</small>}{delivery.chatMessagesSent > 0 && <small>{delivery.chatMessagesSent} chat {delivery.chatMessagesSent === 1 ? "message confirmed" : "messages confirmed"}</small>}{delivery.error && <p className="bridge-validation-error">{delivery.error}</p>}</div><Badge status={delivery.status}/><div className="bridge-inline-actions">{accountId && section === "scheduled" && pending.includes(delivery.status) && <><button className="bridge-icon-button" aria-label={`Move ${delivery.accountName} delivery earlier`} disabled={busy || selectedQueue[0]?.id === delivery.id} onClick={() => move(delivery.id, -1)}><Icon name="up" size={17}/></button><button className="bridge-icon-button" aria-label={`Move ${delivery.accountName} delivery later`} disabled={busy || selectedQueue.at(-1)?.id === delivery.id} onClick={() => move(delivery.id, 1)}><Icon name="down" size={17}/></button></>}{delivery.url && <a className="bridge-icon-button" href={delivery.url} target="_blank" rel="noreferrer" aria-label="View published post"><Icon name="external" size={17}/></a>}{failed.includes(delivery.status) && <button className="bridge-button secondary small" onClick={() => { setError(""); setConfirmed(false); setAction({ type: "retry", item: delivery }); }}>Retry</button>}</div></div>)}</div><div className="bridge-post-actions">{post.status === "draft" ? <button className="bridge-button secondary small" onClick={() => onEditDraft?.(post)}>Continue editing</button> : post.editable && <button className="bridge-button secondary small" onClick={() => { setError(""); setEditing(structuredClone(post)); }}>Edit post</button>}{canCancelRemaining(post) && <button className="bridge-text-button" onClick={() => setAction({ type: "cancel", item: post })}>Cancel remaining</button>}{post.deletable && !post.deliveries.some(item => ["published", "awaiting_publish"].includes(item.status) || item.chatMessagesSent > 0) && <button className="bridge-icon-button" onClick={() => setAction({ type: "delete", item: post })} aria-label={post.status === "draft" ? "Delete draft" : "Delete queued post"}><Icon name="trash" size={17}/></button>}</div></article>)}</div>}
    {action && <Modal title={action.type === "retry" ? "Retry this delivery" : action.type === "cancel" ? "Cancel queued deliveries" : deletingDraft ? "Delete draft" : "Delete queued post"} onClose={() => setAction(null)} busy={busy}><Alert message={error}/><p>{action.type === "retry" ? `Meadow will retry only the delivery to ${action.item.accountName}.${action.item.chatMessagesSent ? ` The first ${action.item.chatMessagesSent} confirmed chat messages will not be sent again.` : ""}` : action.type === "cancel" ? "Pending deliveries for this post will be removed from the queue. Published posts and media already sent to TikTok remain in history." : deletingDraft ? "Permanently remove this saved draft from the project? Uploaded media will remain in your media library." : "Remove this post and its unpublished deliveries from the project?"}</p>{action.type === "retry" && action.item.status === "needs_review" && <Check checked={confirmed} onChange={event => setConfirmed(event.target.checked)}>{retryConfirmation(action.item)}</Check>}<div className="bridge-modal-actions"><button className="bridge-button secondary" onClick={() => setAction(null)}>Go back</button><button className={`bridge-button ${action.type === "retry" ? "" : "danger"}`} disabled={busy || action.type === "retry" && action.item.status === "needs_review" && !confirmed} onClick={perform}>{busy ? "Saving…" : action.type === "retry" ? "Retry delivery" : action.type === "delete" ? deletingDraft ? "Delete draft" : "Delete post" : "Cancel deliveries"}</button></div></Modal>}
