...
export default function PostsQueue({ project, accounts: initialAccounts, media, catalog, onCreate, onUpload, section = "posts" }) {
  ...
  async function uploadEditingMedia(files) {
    ...
    try {
      const uploaded = await onUpload([...files].slice(0, Math.max(0, 35 - editing.mediaIds.length)));
