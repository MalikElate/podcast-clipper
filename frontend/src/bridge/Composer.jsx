...
const SHOW_BATCH_CONTROLS = false;
...
export function PostEditor({ post, onChange, accounts, media, catalog, onPickMedia, uploading = false, compact = false }) {
...
export default function Composer({ project, accounts: initialAccounts, media, catalog, config, onAccounts, onSubmitted, seed, scheduledDate = "", onClearSeed, onUpload }) {
  const [items, setItems] = useState(() => seed?.length ? seed.map(item => ({ ...makePost(project, [item.id], [], scheduledDate), title: item.metadata?.title || item.filename.replace(/\.[^.]+$/, "") })) : [makePost(project, [], [], scheduledDate)]);
  ...
  async function uploadMedia(files) {
    ...
    try {
      ...
      const uploaded = await onUpload([...files].slice(0, available));
      if (alive.current && uploaded.length) changeItems(items.map((item, index) => index === active ? { ...item, mediaIds: [...new Set([...item.mediaIds, ...uploaded.map(upload => upload.id)])].slice(0, 35) } : item));
    }
