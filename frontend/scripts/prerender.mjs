...
try {
  ...
  for (const [path, kind] of [["", null], ["404", "not-found"], ["pricing", "pricing"], ["terms", "terms"], ["terms-of-service", "terms"], ["privacy", "privacy"], ["privacy-policy", "privacy"]]) {
    const html = template.replace('<div id="root"></div>', () => `<div id="root">${render(kind)}</div>`)
      .replace(/<title>.*?<\/title>/, `<title>${kind === "terms" ? "Terms of Service · Meadow" : kind === "privacy" ? "Privacy Policy · Meadow" : kind === "pricing" ? "Pricing · Meadow" : kind === "not-found" ? "Page not found · Meadow" : "Meadow — Publish across every social media"}</title>`);
